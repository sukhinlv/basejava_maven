...
<section>
  <form name="mainForm" method="post" action="resume" enctype="application/x-www-form-urlencoded">
    ...
    <dl>
      <dt>Имя:</dt>
      <dd><input type="text" name="fullName" id="fullName" size=50 value="${resume.fullName}" autofocus required></dd>
    </dl>
...
<%--          https://stackoverflow.com/questions/8627902/how-to-add-a-new-line-in-textarea-element--%>
          <textarea rows="15" cols="120" name="${type.name()}" wrap="soft">${fn:join(listSection.data.toArray(), "&#13&#10;")}</textarea> <br/>
        </c:when>
        ...
        <c:when test="${type eq 'EXPERIENCE' || type eq 'EDUCATION'}">
          ...
          <button onclick="CreateOrganizationSection('${sectionId}', '${type.name()}')" type="button">Добавить организацию</button>
          <br>
          <div id="${sectionId}">
            ...
            <c:forEach var="organizationSectionEntry" items="${organizationSection.data}">
              ...
              <div id="${organizationId}">
                ...
                <dl>
                  <dt>Организация:</dt>
                  <dd><input type="text" name="${type.name()}" size=80 value="${organizationSectionEntry.title}" required></dd>
                  <dd><button onclick="document.getElementById('${organizationId}').remove()" type="button">X</button></dd>
                </dl>
                <dl>
                  <dt>Сайт:</dt>
                  <dd><input type="text" name="${type.name()}" size=80 value="${organizationSectionEntry.website}"></dd>
                </dl>

                <button onclick="CreateOrganizationEntry('${organizationId}', '${type.name()}')" type="button">Добавить место</button>
                <c:forEach var="organizationEntry" items="${organizationSectionEntry.periods}">
                  ...
                  <div id="${organizationEntryId}" class="organizationEntry">
                    <input type="hidden" name="${type.name()}_ENTRY" size=80 value="${organizationId}">
                    <dl>
                      <dt>Профессия/должность:</dt>
                      <dd><input type="text" name="${type.name()}_ENTRY" size=80 value="${organizationEntry.title}" required></dd>
                      <dd><button onclick="document.getElementById('${organizationEntryId}').remove()" type="button">X</button></dd>
                    </dl>
                    <dl>
                      <dt>Описание:</dt>
                      <dd><input type="text" name="${type.name()}_ENTRY" size=80 value="${organizationEntry.desc}"></dd>
                    </dl>
                    <dl>
                      ...
                      <c:if test="${DateUtil.NOW != organizationEntry.endDate}">
                        <dd><input type="text" name="${type.name()}_ENTRY" size=10
                                   ...
                                   placeholder='ГГГГ.ММ'  pattern="[1-2][0-9]{3}.[0-9]{1,2}"></dd></c:if>
                      <c:if test="${DateUtil.NOW == organizationEntry.endDate}">
                        <dd><input type="text" name="${type.name()}_ENTRY" size=10
                                   value=""
                                   placeholder='ГГГГ.ММ'  pattern="[1-2][0-9]{3}.[0-9]{1,2}"></dd></c:if>
